/*
# Super-admin role (restored)

Originally shipped as `20260831090000_super_admin.sql`; that file was removed
by an accidental revert on main and is re-applied here under a later
timestamp so it lands after the migrations that shipped in the meantime.
Everything below is idempotent, so re-running it on a database that already
